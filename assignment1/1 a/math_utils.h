...
public:
	...
	void InitCameraTransform(const Vector3f& Pos, const Vector3f& Target, const Vector3f& Up) {
		/* vector	  up : y+, 
					right: x+, 
				 backword: z+ also known as direction 
		*/

		// target is basically cameraPos
		// camDirection : cameraPos - cameraTarget (it is origin) = camreaPos
		// even though we say direction it points to oposite direction 
		// its more like cameraPos 

		// camera belongs to yz plane 
		// hence up * target gives us positive x-axis of the camera space
		// camRight : up * target

		// now if we multiply z axis vector to x axis of the camera space
		// we will get y axis of the camera space
		// camUp = camDirection * camRight

		Vector3f N = Pos - Target; // this is now camDirection
		N.Normalize();
		Vector3f U = Up;
		U.Normalize();
		U = U.Cross(N); // this is now camRight = up * target
		Vector3f V = N.Cross(U); // this is now camUp = camDirection * camRight
	...
	// we use a right hand coordinate system, the z-coordinates of all points visible by the camera are negative, 
	// which is the reason we use -z instead of z
	// left, right, bottom, top, znear, zfar, factor
	void InitOrthoProjTransform(float l, float r, float b, float t, float zn, float zf, float x = 1.0 ) {
...
// y = f(x) = Ax + b will be written as a augmented matrix and augmented vector
/*
    [y] = [A | b] * [x]
    [1]   [0 | 1]   [1]
        augmented 
...
// return y after affine transformation
Vector3f affineTransform(Vector3f &x, Matrix4f &A){
    Vector4f xAug = Vector4f(x.x, x.y, x.z, 1); // augmented x
    float yAug[4]; 
    for(int i = 0; i < 4; i++){
        yAug[i] = A.m[i][0] * xAug.x + A.m[i][1] * xAug.y + A.m[i][2] * xAug.z + A.m[i][3] * xAug.w; 
    }
    return Vector3f(yAug[0], yAug[1], yAug[2]);
}
