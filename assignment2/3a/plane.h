...
struct Vertex {
    ...
    Vertex() { }
    Vertex(Vector3f p){
        Position = p; Scalar = Vector3f(1, 1, 1); 
    }
};
...
public:
    // isocontoru
    unsigned int isoVAO, isoVBO; float isoVal = 50; 
    vector<Vertex>       isovertices;
    ...
    void readMesh(const char * OffFile)
    {
        ...
        for(int y = -cubeY; y < cubeY ; y++){
            for(int x = -cubeX; x < cubeX ; x++){
                ...
                ver.Scalar = Vector3f( scalarVal , 0 , 1 - scalarVal );
    ...
    void Draw()  // Shader &shader
    {
        ...
        glDrawArrays(GL_LINES, 0, isovertices.size() );
        glBindVertexArray(0);
...
private:
    ...
    void computeIso(int i0, int i1, int i2){
        float c0 = vertices[i0].Scalar.x * 256; 
        float c1 = vertices[i1].Scalar.x * 256; 
        float c2 = vertices[i2].Scalar.x * 256; // cout << c0 << "-" << c1 << "-" << c2 << endl;
        ...
        // p1.Print(); p2.Print(); cout << endl; 

        isovertices.push_back(Vertex(p1)); isovertices.push_back(Vertex(p2));

    }

    Vector3f findPoint(float c1, float c2, int p1, int p2) {
        Vector3f P1 = vertices[p1].Position, P2 = vertices[p2].Position, P3;
        P3 = P1 + (P2 - P1) * ((c1 - isoVal) / (c1 - c2) ) ;
        return P3; 
    }
