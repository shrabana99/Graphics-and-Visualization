...
Shader boxShader, surfaceShader;  
boundingBox box;

const char* offFileName2 = "../Data/bonsai.off";
...
/* pre:  glut window has been initialized
   post: model has been initialized */ {
	...
	surfaceShader.CompileShaders("shaderP.vs", "shaderP.fs");  cout << "shader ok\n"; 
...
static void onDisplay() {
	...
	wRotate.InitRotateTransform(0, rotation, 0);
	...
	id = surfaceShader.use();
	// set the matrix in vertex shader
	wScale.InitScaleTransform(1.0 / surface.cubeX, 1.0 / surface.cubeY, 1.0 / surface.cubeZ); 
	wTranslate.InitTranslationTransform(-0.5, -0.5, -0.5);
	World = wRotate * wTranslate * wScale;
